...
export default function Toolbar() {
  ...
  return (
    <div className="flex items-center gap-1.5 px-3 py-1.5 bg-[var(--bg-secondary)] border-b border-[var(--border-secondary)] text-xs select-none">
      ...
      {/* ── Group 2: Page actions (tools + add/dup/delete) ────────── */}
      <Divider />
      <button
      ...
      <button
        onClick={() => useCanvaStore.getState().duplicatePage()}
      ...
      <button
        onClick={() => {
          const { pages, currentPageIndex, deletePage } = useCanvaStore.getState();
      ...
      {/* ── Group 3: View (zoom/ratio) ───────────────────────────── */}
      <Divider />
      <span className="px-2 py-0.5 rounded bg-[var(--bg-tertiary)] text-[var(--accent-text)] font-bold text-[10px]">{ratioId}</span>
      ...
      {/* Keyboard hints */}
      <div className="hidden lg:flex items-center gap-2 ml-auto text-[9px] text-[var(--text-muted)]">
        <span>Del=hapus</span>
...
function Divider() {
  return <div className="w-px h-5 bg-[var(--border-secondary)] mx-1" />;
}
